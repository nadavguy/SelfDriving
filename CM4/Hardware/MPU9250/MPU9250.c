...
void MPU9250_SetAccelRange(tMPU9250 mpuToInit, AccelRange range)
{
	writeRegister(mpuToInit, ACCEL_CONFIG, range);
}

/* sets the gyro full scale range to values other than default */
void MPU9250_SetGyroRange(tMPU9250 mpuToInit, GyroRange range)
{
	writeRegister(mpuToInit, GYRO_CONFIG, range);
}

/* sets the DLPF bandwidth to values other than default */
void MPU9250_SetDLPFBandwidth(tMPU9250 mpuToInit, DLPFBandwidth bandwidth)
{
	writeRegister(mpuToInit, ACCEL_CONFIG2,bandwidth);
	writeRegister(mpuToInit, CONFIG,bandwidth);
}

/* sets the sample rate divider to values other than default */
void MPU9250_SetSampleRateDivider(tMPU9250 mpuToInit, SampleRateDivider srd)
{
	/* setting the sample rate divider to 19 to facilitate setting up magnetometer */
	writeRegister(mpuToInit, SMPDIV,19);
	...
	if(srd > 9)
	{
		...
		writeAK8963Register(mpuToInit, AK8963_CNTL1,AK8963_CNT_MEAS1);
